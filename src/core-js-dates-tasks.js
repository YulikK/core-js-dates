...
function getWeekNumberByDate(date) {
  ...
  const days = Math.ceil(
    (date - firstDay) / 1000 / 60 / 60 / 24 + firstDay.getDay() + 1
  );
...
function getNextFridayThe13th(date) {
  ...
  let nextFridayThe13th = new Date(Date.UTC(year, month, 13));
  ...
  while (nextFridayThe13th.getDay() !== 5) {
    ...
    nextFridayThe13th = new Date(Date.UTC(year, month, 13));
  }
  return new Date(
    nextFridayThe13th.getTime() + nextFridayThe13th.getTimezoneOffset() * 60000
  );
}
